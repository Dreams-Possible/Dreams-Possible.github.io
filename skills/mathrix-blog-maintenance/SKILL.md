...
- Site content lives in `data/projects.js`. `index.html` and `projects.html` render from that one list through `site.js`.
- `CNAME` maps GitHub Pages to `mathrix.eu.org`; preserve it.
- Homepage picks four projects with equal chance on each visit. Do not add ranking, stars, or a `featured` flag. The project page supports timeline, category, and search.
...
3. Add one object to `window.MATHRIX_PROJECTS` in `data/projects.js`: exact repository `name`, `date` from GitHub `pushed_at` (`YYYY-MM-DD`), one existing `category`, and a short Chinese `description`. The file derives the URL from the repository name. Exclude this website's own repository from the public project list.
...
- Use a concrete one-sentence description: what the project does, with a key technology or context if useful.
